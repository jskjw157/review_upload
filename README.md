...
## 🚀 TypeScript 프로토타입 실행 방법

1. 의존성 설치: `npm install`
2. 번들 빌드: `npm run build`
3. `index.html`을 브라우저에서 열어 번들된 `dist/main.js`와 함께 프로토타입을 확인합니다.

> UI 상호작용 로직은 `src/renderer/main.ts`에서 TypeScript로 정의되며, esbuild로 `dist/main.js`에 번들됩니다. 개발 중에는 `npm run dev`로 워치를 켜 두고, `npm run typecheck`로 타입 검사를 수행할 수 있습니다.
